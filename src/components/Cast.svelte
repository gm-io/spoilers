    ...
    <div class="grid grid-cols-6 gap-4">
        {#key cast_10}
            {#each cast_10 as cast}
                <div class="flex flex-col border-2 rounded border-black">
                    {#if cast.profile_path}
                        <img class="rounded-full h-10 w-10" src={`https://image.tmdb.org/t/p/original${cast.profile_path}`} alt={`${cast.name} profile`}>
                    {/if}
                    <div class="">{cast.name}</div>
                    <div class="">{cast.character}</div>
                </div>
            {/each}
        {/key}
    </div>
...
<div class="w-full flex flex-col">
    <div class="flex bg-orange-7 pl-1 mt-4 mb-2 items-center opacity-70 dark:opacity-100 w-fit pr-2 rounded">
        <!-- <div class="border-gray-300 border-opacity-10 border-b mt-6"></div> -->
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 text-white dark:text-primary-300 opacity-100 dark:opacity-70">
    ...
    <div class="w-full px-4 mx-auto">
        <div class="border-gray-300 border-opacity-0 mb-2 border-b"></div>
